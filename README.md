...
- **DICOM 傳送端 (SCU)**：支援將本地選取的 DICOM 檔案或**直接從本地資料庫挑選檔案**，推播 (C-STORE) 至遠端伺服器 (如 PACS)。傳送前會自動解壓縮並動態偵測所需的 SOP Class，確保相容性最大化。
- **連線測試 (C-ECHO)**：提供一鍵 Ping 遠端 DICOM 節點，測試網路連線狀態 (Verification SOP Class)。
- **連線節點管理 (Peers)**：內建連線對象儲存功能，自動將常用 AE Title、IP 與 Port 設定**永久保存在本地 SQLite 資料庫**中，提供下拉選單快速切換，並支援一鍵刪除已儲存的配置。
...
- **標籤編輯器**：內建防呆機制的編輯介面，支援文字/數值/Multi-value (反斜線分隔) 型別的安全覆寫。自動排除不安全的 VR 類型 (OB, OW, OF, OD, SQ, UN) 與 PixelData 等標籤。新增 **SOP UID 亂數產生器**，產生後顯示於獨立文字框供快速複製。
...
- **安全刪除機制**：執行整筆檢查或單張影像刪除時，**系統會自動判斷檔案來源**。若是透過 SCP 接收至內部 `dicom_files` 目錄的檔案會連同實體檔案一併刪除；若是外部手動匯入的檔案，則僅移除資料庫紀錄而**保留原始實體檔案**，防止誤刪使用者珍貴資料。
...
- **DICOM Sender (SCU)**: Supports pushing locally selected DICOM files or **selecting files directly from the local database** to remote servers (e.g., PACS) via C-STORE. Automatically decompresses and dynamically detects required SOP Classes before transmission to maximize compatibility.
...
- **Peer Management**: Built-in connection saving feature persistent via the **local SQLite database**, automatically recording and quickly switching commonly used AE Title, IP, and Port settings. Also supports one-click deletion of saved configurations.
...
- **Tag Editor**: Built-in foolproof editing interface supporting text/numeric/multi-value (backslash separated) types for safe overwriting. Automatically excludes unsafe VR types (OB, OW, OF, OD, SQ, UN) and tags like PixelData. Includes a **Random SOP UID Generator** that displays generated UIDs in a read-only textbox for easy copying.
...
- **Safe Deletion Mechanism**: When deleting an entire study or a single image, the system automatically checks the file's origin. It permanently deletes physical files only if they are located inside the internal `dicom_files` directory. External files imported manually are preserved on the hard drive (only database records are removed) to prevent accidental data loss.
