...
class DicomEditorDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        btn_generate_uid = QPushButton("🎲 產生新 SOP UID")
        btn_generate_uid.clicked.connect(self._generate_sop_instance_uid)
        btn_generate_uid.setStyleSheet("""
            QPushButton {
                background-color: #03a9f4; color: white; border-radius: 4px; padding: 6px 16px; font-weight: bold;
            }
            QPushButton:hover { background-color: #039be5; }
        """)
        btn_layout.addWidget(btn_generate_uid)
        
        from PyQt6.QtWidgets import QLineEdit
        self.input_generated_uid = QLineEdit()
        self.input_generated_uid.setPlaceholderText("產生的亂數 UID 將顯示於此，方便複製")
        self.input_generated_uid.setReadOnly(True)
        self.input_generated_uid.setStyleSheet("""
            QLineEdit {
                background-color: #f5f5f5; border: 1px solid #e0e0e0;
                border-radius: 4px; padding: 4px 8px; color: #424242; font-family: monospace;
            }
        """)
        btn_layout.addWidget(self.input_generated_uid, stretch=1)
    ...
    def _generate_sop_instance_uid(self):
        """產生一個新的亂數 UID，放入文字輸入框供使用者複製"""
        from pydicom.uid import generate_uid
        
        new_uid = str(generate_uid())
        self.input_generated_uid.setText(new_uid)
        self.input_generated_uid.selectAll()
        self.input_generated_uid.setFocus()
