...
class DatabaseManager:
    ...
    def _create_table(self):
        ...
        """
        create_peers_table_sql = """
        CREATE TABLE IF NOT EXISTS dicom_peers (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            host TEXT NOT NULL,
            port INTEGER NOT NULL,
            ae_title TEXT NOT NULL,
            UNIQUE(host, port, ae_title)
        );
        """
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(create_table_sql)
            cursor.execute(create_peers_table_sql)
            conn.commit()
            logger.debug("資料表 dicom_records 與 dicom_peers 已確認建立")
    ...
    # ==========================================
    # DICOM Peers (遠端伺服器設定) 管理
    # ==========================================
    def get_all_peers(self) -> list:
        """取得所有遠端 DICOM Server 的設定"""
        query_sql = "SELECT host, port, ae_title FROM dicom_peers ORDER BY id ASC;"
        with self._get_connection() as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute(query_sql)
    ...
    def insert_peer(self, host: str, port: int, ae_title: str) -> bool:
        """
        新增一個遠端 DICOM Server 設定。
        若該設定 (host, port, ae_title) 已存在則忽略。
        :return: 是否成功新增 (若已存在則回傳 False)
        """
        insert_sql = """
        INSERT OR IGNORE INTO dicom_peers (host, port, ae_title)
        VALUES (?, ?, ?)
        """
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(insert_sql, (host, port, ae_title))
            conn.commit()
            return cursor.rowcount > 0

    def delete_peer(self, host: str, port: int, ae_title: str) -> int:
        """
        刪除指定的遠端 DICOM Server 設定。
        """
        delete_sql = "DELETE FROM dicom_peers WHERE host = ? AND port = ? AND ae_title = ?"
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(delete_sql, (host, port, ae_title))
            conn.commit()
            deleted_count = cursor.rowcount
            if deleted_count > 0:
                logger.info(f"已從資料庫刪除 Peer — {ae_title}@{host}:{port}")
            return deleted_count
